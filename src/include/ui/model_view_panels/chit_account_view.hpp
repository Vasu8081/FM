...
public:
    ChitAccountView(wxWindow* parent, std::shared_ptr<ChitAccount> model) : AccountView(parent, model) {
        ...
        wxStaticBox* staticBox = new wxStaticBox(this, wxID_ANY, "Chit Account Details");
        wxStaticBoxSizer* staticBoxSizer = new wxStaticBoxSizer(staticBox, wxVERTICAL);

        _chit_name = new wxStaticText(this, wxID_ANY, "Chit Name: " + model->getName());
        ...
        staticBoxSizer->Add(_chit_name, 0, wxEXPAND | wxALL, 5);
    ...
    void update() override {
        auto model = std::dynamic_pointer_cast<ChitAccount>(_model);
        _paid_amount->SetLabelText(std::to_string(model->getPaidAmount()));
    }


private:
    wxStaticText* _chit_name;
